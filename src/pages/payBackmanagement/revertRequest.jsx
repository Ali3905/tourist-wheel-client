...
import { columnsForRevertRefundTable, dummyDataForRevertRefund } from '../../jsonData/tableData';
import CustomTablePagination from '../../components/CustomTablePagination';

const RevertRequest = () => {
  const [filteredData, setFilteredData] = useState(dummyDataForRevertRefund);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForRevertRefundTable = columnsForRevertRefundTable
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = dummyDataForRevertRefund.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "51%" }}>
          ...
          <Filter columns={columnsForRevertRefundTable} onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForRevertRefundTable} data={filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage)} />
