...
import { columnsForTransferHistory, dummyDataForTransferHistory } from '../jsonData/tableData';
import CustomTablePagination from '../components/CustomTablePagination';

const TransferHistory = () => {
  const [filteredData, setFilteredData] = useState(dummyDataForTransferHistory);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForTransferHistory = columnsForTransferHistory
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = dummyDataForTransferHistory.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "65%" }}>
          ...
          <Filter columns={columnsForTransferHistory} onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForTransferHistory} data={filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage)} />
