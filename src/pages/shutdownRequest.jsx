...
import { columnsForShutDown , dummyDataForShutDown  } from '../jsonData/tableData';
import CustomTablePagination from '../components/CustomTablePagination';

const ShutdownRequest = () => {
  const [filteredData, setFilteredData] = useState(dummyDataForShutDown );
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForShutDown  = columnsForShutDown 
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = dummyDataForShutDown.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "51%" }}>
          ...
          <Filter columns={columnsForShutDown } onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForShutDown } data={filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage)} />
...
export default ShutdownRequest;
