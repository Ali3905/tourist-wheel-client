...
import { columnsForOrders, dataForOrders } from '../../jsonData/tableData';
import CustomTablePagination from '../../components/CustomTablePagination';
import AddButton from '../../components/addButton';

const OnlineOrders = () => {
  const [filteredData, setFilteredData] = useState(dataForOrders);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForOrders = columnsForOrders
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = dataForOrders.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "65%" }}>
          ...
          <Filter columns={columnsForOrders} onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForOrders} data={filteredData} />
