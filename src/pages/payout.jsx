...
import { columnsForPayoutTable, dummyDataForPayout } from '../jsonData/tableData';
import CustomTablePagination from '../components/CustomTablePagination';

const Payout = () => {
  const [filteredData, setFilteredData] = useState(dummyDataForPayout);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForPayoutTable = columnsForPayoutTable
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = dummyDataForPayout.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "51%" }}>
          ...
          <Filter columns={columnsForPayoutTable} onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForPayoutTable} data={filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage)} />
