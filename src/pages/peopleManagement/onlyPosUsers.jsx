...
import { columnsForSellerTable, SellerDummyData } from '../../jsonData/tableData';
import CustomTablePagination from '../../components/CustomTablePagination';

const OnlyPosUsers = () => {
  const [filteredData, setFilteredData] = useState(SellerDummyData);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForSellerTable = columnsForSellerTable
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = SellerDummyData.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "65%" }}>
          ...
          <Filter columns={columnsForSellerTable} onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForSellerTable} data={filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage)} />
