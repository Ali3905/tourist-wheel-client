...
import { columnsForAdminTable, adminDummyData } from '../../jsonData/tableData';
import CustomTablePagination from '../../components/CustomTablePagination';

const SheduledOrders = () => {
  const [filteredData, setFilteredData] = useState(adminDummyData);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForAdminTable = columnsForAdminTable
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = adminDummyData.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "65%" }}>
          ...
          <Filter columns={columnsForAdminTable} onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForAdminTable} data={filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage)} />
