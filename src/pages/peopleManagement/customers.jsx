...
import { columnsForCustomerTable, CustomerDummyData } from '../../jsonData/tableData';
import CustomTablePagination from '../../components/CustomTablePagination';

const Customers = () => {
  const [filteredData, setFilteredData] = useState(CustomerDummyData);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForCustomerTable = columnsForCustomerTable
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = CustomerDummyData.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "52%" }}>
          ...
          <Filter columns={columnsForCustomerTable} onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForCustomerTable} data={filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage)} />
