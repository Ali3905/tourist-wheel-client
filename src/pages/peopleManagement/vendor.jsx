...
import { columnsForVendorTable, VendorDummyData } from '../../jsonData/tableData';
import CustomTablePagination from '../../components/CustomTablePagination';

const Vendor = () => {
  const [filteredData, setFilteredData] = useState(VendorDummyData);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const filteredcolumnsForVendorTable = columnsForVendorTable
  ...
  const handleSearchChange = (value) => {
    const newFilteredData = VendorDummyData.filter((item) =>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: "flex-end", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "65%" }}>
          ...
          <Filter columns={columnsForVendorTable} onFilterChange={handleFilterChange} />
        </div>
      </div>
      <div style={{ backgroundColor: 'white', borderRadius: '5px', margin: '20px 0px' }}>
        ...
        <CustomTable columns={filteredcolumnsForVendorTable} data={filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage)} />
